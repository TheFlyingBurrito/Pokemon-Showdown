...
var cmds = {
	...
	poll: function(target, room, user) {
		...
		room.addRaw('<div class="infobox"><h2>' + tour[room.id].question + separacion + '<font class="closebutton" size=1><small>/vote OPTION</small></font><br><font size="1" target="_blank"><i>' + sanitize(user.name) + ' started the poll.</i></font></h2><hr />' + separacion + separacion + " &bull; " + tour[room.id].answerList.join(' &bull; ') + '</div>');
	},
